...
const SUMMARY_SYSTEM_PROMPT =
  ...
  "บทความหลายชิ้นมักพูดถึง 'ผลการตรวจ' คุณภาพน้ำ/ตะกอนดินรอบต่างๆ (เช่น ครั้งที่ 3, ครั้งที่ 10) ซึ่งแต่ละบทความอาจรายงานผลตรวจคนละรอบคนละวันที่ — ห้ามหยิบตัวเลขจากผลตรวจที่มีอายุเกิน 30 วันนับจากวันสิ้นสุดสัปดาห์ของรายงานนี้มาบรรยายราวกับเป็นสถานการณ์ปัจจุบัน ให้ใช้เฉพาะผลตรวจครั้งล่าสุดที่มีอายุไม่เกิน 30 วันเป็นตัวแทนสถานการณ์ตอนนี้ ผลตรวจที่เก่ากว่านั้นใช้ได้เฉพาะตอนเปรียบเทียบแนวโน้ม/ความคืบหน้าเท่านั้น และต้องระบุวันที่ของผลตรวจนั้นกำกับไว้เสมอเพื่อไม่ให้เข้าใจผิดว่าเป็นค่าล่าสุด. " +
  "ทุกครั้งที่อ้างอิงข่าวในเนื้อหา ต้องใส่เป็นลิงก์ markdown ที่คลิกได้ทันที ไม่ใช่ตัวเลขเปล่าๆ. " +
  "ถ้าจะทำตาราง ให้ใช้ GitHub-flavored markdown table (แถวหัวตาราง, แถวคั่น |---|---|, แถวข้อมูล) เท่านั้น ห้ามใช้รูปแบบตารางอื่น";
...
export async function runWeeklySummary(env: Env): Promise<{ summary: string; count: number }> {
  ...
  const prompt = `วันนี้/วันสิ้นสุดสัปดาห์ของรายงานนี้คือ ${weekEndIso.slice(0, 10)} — ใช้วันนี้เป็นจุดอ้างอิงคำนวณอายุของ "ผลการตรวจ" ทุกรายการ

นี่คือข่าวของสัปดาห์ ${weekStartIso.slice(0, 10)} ถึง ${weekEndIso.slice(0, 10)} เกี่ยวกับสารพิษปนเปื้อนแม่น้ำกก แม่น้ำสาย แม่น้ำโขง และแม่น้ำสาละวิน:
...
- **ผลการตรวจคุณภาพน้ำ/ตะกอนดิน**: เทียบวันที่ของผลตรวจแต่ละรอบกับวันที่ ${weekEndIso.slice(0, 10)} ก่อนนำมาเขียน — ถ้าผลตรวจนั้นเก่ากว่า 30 วัน **ห้ามบรรยายเป็นสถานการณ์ล่าสุด/ปัจจุบัน** ให้หยิบเฉพาะผลตรวจรอบล่าสุดที่อายุไม่เกิน 30 วันมาเป็นตัวแทนสถานการณ์ตอนนี้ (ถ้าไม่มีเลยให้ระบุตรงๆ ว่า "ยังไม่มีผลตรวจใหม่ในช่วงนี้ ข้อมูลล่าสุดที่มีคือวันที่ ...") ผลตรวจรอบเก่าใช้ได้แค่ตอนเทียบแนวโน้มและต้องกำกับวันที่ไว้ทุกครั้ง
...
- ถ้าจะสรุปเป็นตาราง ใช้ GitHub-flavored markdown table เท่านั้น (แถวหัวตาราง / แถวคั่น |---|---| / แถวข้อมูล ให้ครบทุกแถว จำนวนคอลัมน์เท่ากันทุกแถว)
